...
macro(rossdl_generate_code target)
  ...
  message(STATUS "rossdl_generate_code() started ===================================")
  ...
  foreach(_file ${_ARG_UNPARSED_ARGUMENTS})
    if(IS_ABSOLUTE "${_file}")
      ...
      list(APPEND _code_tuples "${_file}")
    else()
      ...
      list(APPEND _code_tuples "${CMAKE_CURRENT_SOURCE_DIR}:${_file}")
    endif()
  endforeach()

  message(STATUS "_code_tuples: ${_code_tuples}")
  ...
  foreach(_tuple ${_code_tuples})
    string(REGEX REPLACE ":([^:]*)$" "/\\1" _abs_file "${_tuple}")
    message(STATUS "    File In: ${_abs_file}")

    set(_header_out_file  ${CMAKE_CURRENT_BINARY_DIR}/include/${PROJECT_NAME}/Nodes.hpp)
    message(STATUS "    Header DIR Out: ${_header_out_file}")

    set(_cpp_out_file  ${CMAKE_CURRENT_BINARY_DIR}/src/${PROJECT_NAME}/Nodes.cpp)
    message(STATUS "    CPP DIR Out: ${_cpp_out_file}")
  
    add_custom_command(
      OUTPUT ${_cpp_out_file} ${_header_out_file}
      COMMAND Python3::Interpreter
        ARGS sdl_generator_cpp ${_abs_file}
      DEPENDS ${_abs_file}
    ...
    add_library(${PROJECT_NAME}_generated SHARED
      ${_cpp_out_file} ${_header_out_file}
    )
    #   --generator-arguments-file "${generator_arguments_file}"

    #file(STRINGS ${_abs_file} description_file_content)
#
    #message(STATUS "    CONTENT: ${description_file_content}")
#
    #foreach(token ${description_file_content})
    #  message(STATUS "token: ${token}")
    #endforeach()
    
    
    # add_custom_command(
    #   OUTPUT ${_cpp_out_dir}_monada
    #   COMMAND Python3::Interpreter
    #   ARGS sdl_generator_cpp
    #   --generator-arguments-file "${generator_arguments_file}"
    #   COMMAND touch ${_cpp_out_dir}_monada
    #   DEPENDS ${_abs_file}
    #   COMMENT "Generating code for ROS 2 System"
    #   VERBATIM
    # )
    # add_custom_target(generate_files DEPENDS ${_cpp_out_dir}_monada)
  
    #string(REGEX REPLACE ":([^:]*)$" ";\\1" _sdl_list "${_tuple}")
    #list(GET _sdl_list 1 _sdl_relpath)
    #file(TO_CMAKE_PATH "${_sdl_relpath}" _sdl_relpath)
    #list(APPEND _headers "${_sdl_relpath}")
#
    #message(STATUS "    _sdl_relpath: ${_sdl_relpath}")

    # message(STATUS "    Header Out: ${_abs_file}")
